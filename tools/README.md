...
```bash
make -C tools          # build all C tools
make -C tools clean    # remove binaries
```
...
```bash
python3 tools/analyze-capture.py capture.pgm --log capture.log
```
...
```bash
sudo ./tools/install.sh      # copies built .so to /usr/local/lib
sudo ./tools/uninstall.sh    # removes it and restores stock libfprint
```
